...
apa.knit.regression.for.papaja <- function(table_object, latex_font_size){

     table_df <- as_tibble(as.matrix(table_object$table_body))
     ...
     table_out <- papaja::apa_table(table_df,
                            ...
                            caption = table_object$table_title,
                            note = table_note,
                            escape = FALSE)
