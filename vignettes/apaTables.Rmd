...
An inspection of this output reveals the 10 variables/columns in the dataset. The drink_attitude_wide dataset represents a repeated measures design where the repeated measures variable **drink** has 3 levels (beer, wine, water). The second repeated measures variable **imagery** also has three levels (positive, negative, neutral). The dependent variable is **attitude**. The first column contains a code for each participant (P1, P2, etc.)  Note that the **participants** column is a factor as indicated by the "< fct >" descriptor. The participant column must be a factor which can, as noted previously, be created by the **as.factor** command.

The remaining 9 columns represent values for each cell representing a drink/imagery combination of levels. A 3 drink (beer, wine, water) by 3 imagery (positive, negative, neutral) design has 9 cells hence the nine columns. The naming convention for column names is critical to the workflow we describe below. Each column name represents the level of the **drink** variable separated using an underscore ( _ ) from the level of the imagery variables. For example the first column is called **beer_positive** representing the combination of the beer level of drink and the positive level of imagery. Each value in this column represents **attitude** for a participant in this cell. 
