...
test_that("apa.1way.table has correct table.body structure", {
     ...
     expect_equal(nrow(t1$table_body), 3) # Placebo, Low Dose, High Dose
     expect_true("M" %in% names(t1$table_body))
     expect_true("SD" %in% names(t1$table_body))
})

test_that("apa.1way.table returns correct descriptive values", {
     t1 <- apa.1way.table(iv = dose, dv = libido, data = viagra)
     # Placebo group: M = 2.20, SD = 1.30
     expect_equal(t1$table_body$M[1], "2.20")
     expect_equal(t1$table_body$SD[1], "1.30")
     # High Dose group: M = 5.00
     expect_equal(t1$table_body$M[3], "5.00")
})
...
test_that("apa.1way.table show.conf.interval adds CI column", {
     ...
     expect_false("M_95%_CI" %in% names(t1_no_ci$table_body))
     expect_true("M_95%_CI" %in% names(t1_ci$table_body))
     expect_equal(ncol(t1_ci$table_body), ncol(t1_no_ci$table_body) + 1)
})

test_that("apa.1way.table table.number parameter works", {
     t1 <- apa.1way.table(iv = dose, dv = libido, data = viagra, table.number = 7)
     expect_equal(t1$table_number, 7)
})

test_that("apa.1way.table title mentions DV and IV names", {
     t1 <- apa.1way.table(iv = dose, dv = libido, data = viagra)
     expect_true(grepl("Libido", t1$table_title))
     expect_true(grepl("Dose", t1$table_title))
})
